...
int main(int argc, char **argv){
  ...
  auvsl::ControlSystem *control_system = new auvsl::SimpleControlSystem();
  ...
  //lookahead.push_back(Vector2f(waypoints[4], waypoints[5]));

  do{
    control_system->computeVelocityCommand(lookahead, vehicle_pose, v_forward, v_angular);  
    ROS_INFO("Current %f %f   Target %f %f   future %f %f   Control  %f %f", vehicle_pose.position.x, vehicle_pose.position.y,   lookahead[0][0], lookahead[0][1],   lookahead[1][0], lookahead[1][1],   v_forward, v_angular);
    ...
    dx = lookahead[0][0] - vehicle_pose.position.x;
    dy = lookahead[0][1] - vehicle_pose.position.y;
    dist = sqrtf(dx*dx + dy*dy);
    if(dist < .5){
      idx++;
      lookahead[0][0] = waypoints[(idx*2)];
      lookahead[0][1] = waypoints[(idx*2) + 1];
      
      //  lookahead[1][0] = waypoints[(idx*2)];
      //lookahead[1][1] = waypoints[(idx*2) + 1];
